...
static struct menu  helpMenu[] = {
	{ "EOL Homepage", BrowseTo, (XtPointer)1 },
...
static char unsigned annotateBits[] = {
...
static char unsigned pointerBits[] = {
...
Widget CreateMainWindow(Widget parent)
{
  ...
  if (!RealTime)
    ...
    pixmap = XCreatePixmapFromBitmapData(XtDisplay(form1),
           ...
           (char *)pointerBits, 20, 20, fg, bg, DefaultDepthOfScreen(XtScreen(form1)));
    ...
    pixmap = XCreatePixmapFromBitmapData(XtDisplay(form1),
           RootWindowOfScreen(XtScreen(form1)),
           (char *)annotateBits, 20, 20, fg, bg, DefaultDepthOfScreen(XtScreen(form1)));
    ...
    pixmap = XCreatePixmapFromBitmapData(XtDisplay(form1),
           RootWindowOfScreen(XtScreen(form1)),
           (char *)grabBits, 20, 20, fg, bg, DefaultDepthOfScreen(XtScreen(form1)));
    XtSetArg(args[1], XmNlabelPixmap, pixmap);
...
void createPanelButts(Widget parent, std::vector<Widget>& panelB, XtCallbackProc set)
{
  ...
  for (size_t i = 0; i < MAX_PANELS; ++i)
    {
    snprintf(buffer, BUFFSIZE, "%zu", i+1);
